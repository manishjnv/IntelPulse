...
export interface ThreatsClientProps {
  initialData: IntelListResponse | null;
  initialStats: IntelStatsResponse | null;
}

export function ThreatsClient({ initialData, initialStats }: ThreatsClientProps) {
  const searchParams = useSearchParams();
  const [data, setData] = useState<IntelListResponse | null>(initialData);
  const [stats, setStats] = useState<IntelStatsResponse | null>(initialStats);
  ...
  // Server-side initial fetch already covered the default filter set driven by
  ...
  useEffect(() => {
    if (initialStats !== null) return; // server already provided stats; refresh timer below handles updates
    fetchStats();
                ...
                <div className="flex flex-wrap gap-1">
                  {stats.top_tags.slice(0, 20).map((tag) => (
                    <button
                      ...
                      className="text-[10px] bg-muted/50 hover:bg-muted/80 text-muted-foreground border border-border/40 px-1.5 py-0.5 rounded transition-colors cursor-pointer"
                    >
                      {tag}
                    </button>
                  ))}

...
const VendorStatsWidget = dynamic(() => import("./VendorStatsWidget"), {
  ssr: false,
  loading: () => <Skeleton className="h-48 w-full rounded-lg" />,
});
const VulnerableProductsTable = dynamic(() => import("./VulnerableProductsTable"), {
  ssr: false,
  loading: () => <Skeleton className="h-96 w-full rounded-lg" />,
});
const ThreatCampaignsTable = dynamic(() => import("./ThreatCampaignsTable"), {
  ssr: false,
  loading: () => <Skeleton className="h-96 w-full rounded-lg" />,
});
...
// ── Main Page ────────────────────────────────────────────
export interface NewsClientProps {
  initialNews: NewsListResponse | null;
  initialCategories: NewsCategoriesResponse | null;
  initialStats: NewsStatsResponse | null;
  initialPipelineStatus: NewsPipelineStatus | null;
}
...
export default function NewsClient({
  ...
  // Data
  const [news, setNews] = useState<NewsListResponse | null>(initialNews);
  const [categories, setCategories] = useState<NewsCategoriesResponse | null>(initialCategories);
  ...
  // Skip-flags: prevent on-mount client fetches from re-requesting data the
  // server already delivered inline. Subsequent calls (auto-refresh, filter
  // changes, or fallback when server returned null) proceed normally.
  const skipInitialNewsRef = useRef(initialNews !== null);
  ...
  useEffect(() => {
    if (skipInitialCategoriesRef.current) {
      skipInitialCategoriesRef.current = false;
    } else {
      fetchCategories();
    }
  ...
  }, [fetchCategories, fetchStats, fetchExtractionStats]);
  useEffect(() => {
    if (skipInitialNewsRef.current) {
      skipInitialNewsRef.current = false;
      return;
    }
    fetchNews();
  }, [fetchNews]);
  useEffect(() => {
    if (skipInitialPipelineRef.current) {
      skipInitialPipelineRef.current = false;
      return;
    }
    fetchPipelineStatus();
  }, [fetchPipelineStatus]);

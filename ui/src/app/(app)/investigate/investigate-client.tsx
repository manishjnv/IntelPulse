...
/* ── Props ──────────────────────────────────────────── */
export interface InvestigateClientProps {
  /** Pre-fetched graph stats (relationship count, avg confidence). Null = server fetch failed; client will re-fetch. */
  initialStats: GraphStatsResponse | null;
  /** Pre-fetched featured/suggested entities. Null = server fetch failed; client will re-fetch. */
  initialFeatured: api.GraphFeaturedEntity[] | null;
  /**
   * Pre-fetched graph data for a deep-linked entity (when `?id=` is present in
   * the URL at SSR time). Null when there was no `id` param or the fetch failed;
   * client `explore()` call handles loading in that case.
   */
  initialGraphData: GraphResponse | null;
}
...
export function InvestigateClient({
  ...
  const skipInitialFeaturedRef = useRef(initialFeatured !== null && initialFeatured.length > 0);
  // If the server pre-fetched graph data (deep-link), skip the client-side
  // explore() that would otherwise fire on mount via the id-param effect.
  const skipInitialExploreRef = useRef(initialGraphData !== null);
  ...
  useEffect(() => {
    if (skipInitialFeaturedRef.current) {
      skipInitialFeaturedRef.current = false;
      return;
    }
    fetchFeatured();
  }, [fetchFeatured]);

  /* ── Deep-link explore on mount ─────────────────── */
  useEffect(() => {
    if (skipInitialExploreRef.current) {
      // Server already delivered the graph; clear flag so future re-explores work.
  ...
  // only if the user hasn't already typed/deep-linked AND server didn't

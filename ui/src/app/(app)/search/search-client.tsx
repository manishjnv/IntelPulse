...
import type { SearchFilters, IntelItem, SearchResponse } from "@/types";
import {
  searchIntel,
  getSearchStats,
  enrichIOC,
...
export interface SearchClientProps {
  /** Pre-fetched search stats (filter distributions, totals). null = not yet loaded. */
  initialStats: SearchAggStats | null;
  /**
   * Pre-fetched search results for a seeded ?q= URL. null when the landing has
   * no query or when the server fetch failed (client falls back to its own fetch).
   */
  initialSearchResult: SearchResponse | null;
  /**
   * The query string that produced initialSearchResult, so the client can
   * pre-fill the input and skip the duplicate fetch on mount.
   */
  initialQuery: string;
}
...
export default function SearchClient({
  ...
  // useSearchParams stays in the client — the server wrapper reads searchParams
  // from its own prop and passes them down as plain strings.
  ...
  // Local state replaces Zustand store so we can seed from SSR props.
  const [searchResult, setSearchResult] = useState<SearchResponse | null>(initialSearchResult);
  const [searchLoading, setSearchLoading] = useState(false);

  const [stats, setStats] = useState<SearchAggStats | null>(initialStats);
  ...
  // Skip-flags for mount-time effects that the server already satisfied.
  // If the server provided stats, skip the initial getSearchStats() call.
  ...
  useEffect(() => {
    ...
    // Skip the very first fire if the server already fetched this result.
  ...
  // Load stats on mount — skipped when server already provided them.
  useEffect(() => {
  ...
  // This effect handles client-side navigations that land with a query string
  // that was NOT seen by the server (e.g. soft-nav from another page).
  useEffect(() => {
    const q = searchParams.get("q");
    const hunt = searchParams.get("hunt");
    if (q && q.trim()) {
      // If the server already handled this exact query (initialQuery matches),
      // just trigger hunt if requested — don't re-search.
      if (q.trim() === initialQuery && initialSearchResult !== null) {
        if (hunt === "1") {

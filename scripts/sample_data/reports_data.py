"""Rich sample reports — 25 entries spanning every (report_type, severity) pair.

Shared between `scripts/seed_sample_data.py` (DB-direct) and the legacy
`scripts/seed_sample_reports.py` (HTTP curl). Keeping the data in one
module prevents divergence between the two seed paths.
"""

from __future__ import annotations


REPORT_TYPES: list[str] = ["incident", "threat_advisory", "weekly_summary", "ioc_bulletin", "custom"]
SEVERITIES: list[str] = ["critical", "high", "medium", "low", "info"]

# Mapping severity → TLP tier used when none is specified inline.
TLP_FOR_SEVERITY: dict[str, str] = {
    "critical": "TLP:RED",
    "high": "TLP:AMBER",
    "medium": "TLP:GREEN",
    "low": "TLP:GREEN",
...
SAMPLE_REPORTS: dict[str, dict[str, dict]] = {
    "incident": {
        "critical": {

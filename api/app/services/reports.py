...
async def export_pdf(
    ...
    elements.append(HRFlowable(width="100%", thickness=0.5, color=HexColor("#CCCCCC"), spaceAfter=6))
    elements.append(Paragraph(
        f"Generated by IntelPulse · {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}",
        styles["FooterText"],
    ))
    if include_tlp_watermark:
        elements.append(Paragraph(
...
async def export_stix(
    ...
    # Identity (IntelPulse organization)
        ...
        if item.item_type in ("intel", "intel_item"):
            ...
            if cve_ids:
                ...
                stix_objects.append({
                    ...
                    "description": f"Intel item from {meta.get('source_name', 'IntelPulse')}",
...
async def export_html(
    ...
    h.append(f"<title>{report.title} — IntelPulse</title>")
    h.append("""<style>
    ...
    h.append(f"<div class='footer'>Generated by IntelPulse · {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}")
    if include_tlp_watermark:
